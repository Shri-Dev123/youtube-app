import React from "react";
const commentsData = [
    {
        name: "Shrikant Kallshetty",
        text: "Hello shrikant how are you doing...!",
        replies: [
            {
                name: "naagesh Kallshetty",
                text: "Hello naagesh how are you doing...!",
                replies: [
                    {
                        name: "Hanumantrao  Kallshetty",
                        text: "Hello Hanumantrao how are you doing...!",
                        replies: [
                            {
                                name: "akash Kallshetty",
                                text: "Hello akash how are you doing...!",
                                replies: [],
                            },
                        ],
                    },
                ],
            },
        ],
    },
    {
        name: "Shashikant Kallshetty",
        text: "Hello shashikant how are you doing...!",
        replies: [
            {
                name: "naagesh Kallshetty",
                text: "Hello naagesh how are you doing...!",
                replies: [
                    {
                        name: "Hanumantrao  Kallshetty",
                        text: "Hello Hanumantrao how are you doing...!",
                        replies: [
                            {
                                name: "akash Kallshetty",
                                text: "Hello akash how are you doing...!",
                                replies: [],
                            },
                        ],
                    },
                ],
            },
        ],
    },
    {
        name: "Sharada Kallshetty",
        text: "Hello Sharada how are you doing...!",
        replies: [],
    },
    {
        name: "Hanumantrao  Kallshetty",
        text: "Hello Hanumantrao how are you doing...!",
        replies: [
            {
                name: "naagesh Kallshetty",
                text: "Hello naagesh how are you doing...!",
                replies: [
                    {
                        name: "Hanumantrao  Kallshetty",
                        text: "Hello Hanumantrao how are you doing...!",
                        replies: [
                            {
                                name: "akash Kallshetty",
                                text: "Hello akash how are you doing...!",
                                replies: [
                                    {
                                        name: "naagesh Kallshetty",
                                        text: "Hello naagesh how are you doing...!",
                                        replies: [
                                            {
                                                name: "Hanumantrao  Kallshetty",
                                                text: "Hello Hanumantrao how are you doing...!",
                                                replies: [
                                                    {
                                                        name: "akash Kallshetty",
                                                        text: "Hello akash how are you doing...!",
                                                        replies: [],
                                                    },
                                                    {
                                                        name: "naagesh Kallshetty",
                                                        text: "Hello naagesh how are you doing...!",
                                                        replies: [
                                                            {
                                                                name: "Hanumantrao  Kallshetty",
                                                                text: "Hello Hanumantrao how are you doing...!",
                                                                replies: [
                                                                    {
                                                                        name: "akash Kallshetty",
                                                                        text: "Hello akash how are you doing...!",
                                                                        replies:
                                                                            [],
                                                                    },
                                                                ],
                                                            },
                                                        ],
                                                    },
                                                    {
                                                        name: "naagesh Kallshetty",
                                                        text: "Hello naagesh how are you doing...!",
                                                        replies: [
                                                            {
                                                                name: "Hanumantrao  Kallshetty",
                                                                text: "Hello Hanumantrao how are you doing...!",
                                                                replies: [
                                                                    {
                                                                        name: "akash Kallshetty",
                                                                        text: "Hello akash how are you doing...!",
                                                                        replies:
                                                                            [],
                                                                    },
                                                                ],
                                                            },
                                                        ],
                                                    },
                                                ],
                                            },
                                        ],
                                    },
                                ],
                            },
                        ],
                    },
                ],
            },
        ],
    },
    {
        name: "akash Kallshetty",
        text: "Hello akash how are you doing...!",
        replies: [],
    },
    {
        name: "naagesh Kallshetty",
        text: "Hello naagesh how are you doing...!",
        replies: [],
    },
    {
        name: "sachin Kallshetty",
        text: "Hello sachin how are you doing...!",
        replies: [],
    },
];

const Comment = ({ data }) => {
    const { name, text, replies } = data;
    return (
        <div className="flex shadow-sm bg-gray-100 p-2 rounded-lg my-2">
            <img
                className="h-8 w-8"
                alt="user"
                src="data:image/png;base64,"
            />
            <div className="px-3">
                <p className="font-bold">{name}</p>
                <p>{text}</p>
            </div>
        </div>
    );
};

const CommentsList = ({ comments }) => {
    return comments.map((comment, index) => (
        <div>
            <Comment key={index} data={comment} />
            <div className="pl-5 border border-l-black ml-5">
                <CommentsList comments={comment.replies} />
            </div>
        </div>
    ));
};

const CommentsContainer = () => {
    return (
        <div className="m-5 p-2">
            <h1 className="text-2xl font-bold">Comments:</h1>
            <CommentsList className="m-3" comments={commentsData} />
        </div>
    );
};

export default CommentsContainer;
